...
def integrate_v2( # TODO: if dynamic ODE has a dependancy on t, need to modify rk4_step
  ...
  h: float, # step size
  N: int, # steps
  forward: bool = True, # Forward or Backward mode
) -> Tuple[np.ndarray, np.ndarray]:
  # nh : hold u constant for each integration step (zero-order interpolation)
  @jit
  def rk4_step(x_t1, u, u_next):
    convex_approx = (u + u_next)/2
    if forward: hh = h
    else: hh = -1 * h

    k1 = dynamics_t(x_t1, u)
    k2 = dynamics_t(x_t1 + hh*k1/2, convex_approx)
    k3 = dynamics_t(x_t1 + hh*k2/2, convex_approx)
