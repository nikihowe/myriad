...
class FiniteHorizonControlSystem(object):
  ...
  def dynamics(self, x_t: jnp.ndarray, u_t: float, v_t: jnp.ndarray) -> jnp.ndarray:
    raise NotImplementedError
  
  def cost(self, x_t: jnp.ndarray, u_t: float) -> float:
    raise NotImplementedError

  def plot_solution(self, x: jnp.ndarray, u: jnp.ndarray, adj: Optional[jnp.array]) -> None:
    raise NotImplementedError
...
class CartPole(FiniteHorizonControlSystem):
  ...
  def dynamics(self, x_t: jnp.ndarray, u_t: float) -> jnp.ndarray:
    q1, q2, q̇1, q̇2 = x_t
    # Eq. 6.1
    q̈1 = (self.l * self.m2 * jnp.sin(q2) * q̇2**2 + u_t + self.m2 * self.g * jnp.cos(q2) * jnp.sin(q2)) / (self.m1 + self.m2 * (1 - jnp.cos(q2)**2))
    q̈1 = jnp.squeeze(q̈1)
    # Eq. 6.2
    q̈2 = - (self.l * self.m2 * jnp.cos(q2) * q̇2**2 + u_t * jnp.cos(q2) + (self.m1 + self.m2) * self.g * jnp.sin(q2)) / (self.l * self.m1 + self.l * self.m2 * (1 - jnp.cos(q2)**2))
    q̈2 = jnp.squeeze(q̈2)
    return jnp.array([q̇1, q̇2, q̈1, q̈2])
  
  def cost(self, x_t: jnp.ndarray, u_t: float, t: float) -> float:
    # Eq. 6.3
    return u_t ** 2
...
class SEIR(FiniteHorizonControlSystem):
  ...
  def cost(self, y_t: jnp.ndarray, u_t: jnp.float64, t:float) -> jnp.float64:
